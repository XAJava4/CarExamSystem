...
/*		$("#answer").text("");
		$("#answer").append(data1.examAnswer);*/
		$("#examParse").text("");
	...
	/*function getOneExam(id) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$("#id").val(data.examID);
				$("#title").text("");
				$("#title").append(data.examTitle);
				$("#optionA").text("");
				$("#optionA").append(data.examOptionA);
				$("#optionB").text("");
				$("#optionB").append(data.examOptionB);
				$("#optionC").text("");
				$("#optionC").append(data.examOptionC);
				$("#optionD").text("");
				$("#optionD").append(data.examOptionD);
				$("#answer").text("");
				$("#answer").append(data.examAnswer);
				$("#examParse").text("");
				$("#examParse").append(data.examParse)
				var address=data.examImage;
                $("#image").src=address;
	...
	function refer(id,score) {
		
		$.ajax({
			type : 'POST',
			contentType : 'application/json',
			url : CONTEXT + '/addScore',
			processData : false,
			dataType : 'json',
    ...
    var topicBox="<span id='" + exams[x].examID + "'><input type='checkbox' style='width:20px;height:20px;' " +
